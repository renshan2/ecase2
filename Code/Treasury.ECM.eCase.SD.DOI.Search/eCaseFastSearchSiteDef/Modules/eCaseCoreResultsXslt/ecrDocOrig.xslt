  ...
  <xsl:template name="dvt_1.body">
    ...
    <xsl:variable name="dvt_LastRowValue">
      <xsl:for-each select="$Rows">
        <xsl:sort select="ecaseoriginator=''" order="ascending" />
        <xsl:sort select="ecaseoriginator" order="ascending" />
        <xsl:if test="position()=$LastRow">
          <xsl:value-of select="ecaseoriginator" />
        </xsl:if>
      </xsl:for-each>
    </xsl:variable>
    <xsl:variable name="dvt_Rows">
      <root>
        <xsl:for-each select="$Rows">
          <xsl:sort select="ecaseoriginator=''" order="ascending" />
          <xsl:sort select="ecaseoriginator" order="ascending" />
          <xsl:if test="(position() &gt;= $FirstRow and position() &lt;= $LastRow) or (position() &gt; $LastRow and ecaseoriginator = $dvt_LastRowValue)">
            <xsl:copy-of select="." />
          </xsl:if>
        </xsl:for-each>
      </root>
      ...
      <xsl:variable name="NewGroup_0">
        <xsl:choose>
          <xsl:when test="not ($dvt_groupfield)">
            <xsl:value-of select="ddwrt:NameChanged(string(ecaseoriginator), 0)" />
          </xsl:when>
      ...
      <xsl:choose>
        ...
        <xsl:when test="not($dvt_groupfield) and (not($NewGroup_0='') and position() &gt;= $FirstRow and position() &lt;= $LastRow or ($FirstRow = position()))">
          <xsl:variable name="groupheader0">
            <xsl:choose>
              <xsl:when test="not (ecaseoriginator) and (ecaseoriginator) != false()">
                <xsl:value-of select="' '" />
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="ecaseoriginator" />
          ...
          <xsl:call-template name="dvt_1.groupheader0">
            <xsl:with-param name="fieldtitle">ecaseoriginator</xsl:with-param>
            <xsl:with-param name="fieldname">ecaseoriginator</xsl:with-param>
            <xsl:with-param name="fieldvalue" select="$groupheader0" />
            <xsl:with-param name="fieldtype" select="'text'" />
            <xsl:with-param name="nodeset" select="msxsl:node-set($dvt_Rows)/root//Result[((ecaseoriginator)=$groupheader0 or ((not(ecaseoriginator) or ecaseoriginator='') and $groupheader0=' '))]" />
