      ...
      <xsl:choose>
            ...
            <span>
              <ul class="srch-picturetext">
                <li class="srch-Title2 srch-Title5">
                  <a href="{$url}" id="{concat('CSR_',$id)}" title="{title}" target="_blank">
                    <xsl:choose>
        ...
        <xsl:otherwise>
          <div style="float: left;">
            <input type="hidden" id="hdnIsDoc" value="{isdocument}" ></input>
            <input type="checkbox" id="chkSelect" value="{url}" >
            </input>
          </div>
          ...
          <xsl:choose>
            <xsl:when test="$IsThisListScope = 'True' and contentclass[. = 'STS_ListItem_PictureLibrary'] and picturethumbnailurl[. != '']">
              <div style="padding-top: 2px; padding-bottom: 2px;">
                <a href="{$url}" id="{concat($currentId,'_P')}" title="{title}" target="_blank">
